...
class Transaction(object):
    ...
    def from_json(cls, transaction_json):
        trasaction = cls.__init__(transaction_json['source'], transaction_json['destination'], transaction_json['amount'], transaction_json.get('signature', None))
    ...
    def _calculate_tx_hash(self):
        """
        Calculates the sha256 hash of a transaction (source, destination, amount, timestamp, signature)
    ...
    def sign(self, private_key):
        signature = pyelliptic.ECC(curve='secp256k1', privkey=private_key, pubkey=self._source.decode('hex')).sign(self.to_signable()).encode('hex')
        self._signature = signature
        self._tx_hash = self._calculate_tx_hash()
        return signature
    ...
    def to_json(self):
        return json.dumps(self, default=lambda o: {key_lstrip('_'): value for key, value in o.__dict__.items()}, sort_keys=True)
